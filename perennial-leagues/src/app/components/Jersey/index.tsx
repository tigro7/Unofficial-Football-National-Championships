...
const Jersey = ({ colors, icon, dimensions }: { colors: { primary: string; secondary: string;}, icon?: string | null, dimensions?: number}) => {
  ...
  const iconClassString = `text-center ${defaultDimensions >= 16 ? 'bg-black/10' : ''} relative top-${defaultDimensions / 4} md:top-${defaultDimensions / 4} lg:top-${defaultDimensions / 2} 
                            left-${defaultDimensions / 4} md:left-${defaultDimensions / 4} lg:left-${defaultDimensions / 2} 
                            w-${defaultDimensions / 2} md:w-${defaultDimensions / 2} lg:w-${defaultDimensions} 
                            h-${defaultDimensions / 2} md:h-${defaultDimensions / 2} lg:h-${defaultDimensions}
                            pt-2 md:pt-2 lg:pt-4 rounded-full`;
  const minWidthFAIcon = dimensions ? dimensions * 64 : 1024;
  return (
    <div
