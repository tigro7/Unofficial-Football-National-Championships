...
export default async function Page({ params, }: { params: Promise<{ league: string }>}) {
    ...
    const startDate = await fetch (`${host}/api/start/${league}`).then(res => res.json()).then(data => data[0].data);
    const squadre = await fetch (`${host}/api/squadre/${league}`).then(res => res.json());
    const regni = await fetch (`${host}/api/regni/${league}`).then(res => res.json());
    ...
    return (
        <ErrorBoundary>
            ...
            } regni={regni} startDate={startDate} />
        </ErrorBoundary>
    )
}